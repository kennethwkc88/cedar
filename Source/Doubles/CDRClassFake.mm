...
- (BOOL)isKVCCompliantKey:(NSString *)key {
    return [self respondsToSelector:NSSelectorFromString([key componentsSeparatedByString:@"."].firstObject)];
}
...
- (void)handleKVCSelector:(SEL)sel withValue:(id)value forKey:(NSString *)key {
    if ([self isKVCCompliantKey:key]) {
        if ([self has_stubbed_method_for:sel] || !self.requiresExplicitStubs) {
            ...
            [self setValue:value forUnstubbedKey:key];
        }
    } else {
...
- (id)handleKVCSelector:(SEL)sel forKey:(NSString *)key {
    if ([self isKVCCompliantKey:key]) {
        if ([self has_stubbed_method_for:sel] || !self.requiresExplicitStubs) {
            ...
            return [self valueForUnstubbedKey:key];
        }
    } else {
...
- (void)setValue:(id)value forUnstubbedKey:(NSString *)key {
...
- (id)valueForUnstubbedKey:(NSString *)key {
    if (self.requiresExplicitStubs) {
        [[NSException exceptionWithName:NSInternalInconsistencyException
